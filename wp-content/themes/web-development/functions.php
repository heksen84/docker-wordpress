...
function web_development_breadcrumbs() {
    ...
    if (is_home() || is_front_page()) {
        ...
        if ($web_development_showonhome == 1)
            echo '<ul id="breadcrumbs" class="breadcrumb-menubar"><li><a href="' . esc_url(home_url('/')) . '">' . esc_html__('Главная', 'web-development') . '</a></li></ul>';
    } else {

        echo '<ul id="breadcrumbs" class="breadcrumb-menubar"><li><a href="' . esc_url(home_url('/')). '">' . esc_html__('Главная', 'web-development') . '</a> ';
        if (is_category()) {
            ...
            echo  esc_html__('Результаты поиска для ', 'web-development') . ' " ' . get_search_query() . ' "';
        } elseif (is_day()) {
